...
public class Double2ByteOpenCustomHashMap extends AbstractDouble2ByteMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Byte get( final Double ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( strategy.equals( (key[ pos ]), ( k) ) ) ) return (Byte.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private class MapIterator {
  ...
  {
   final boolean used[] = Double2ByteOpenCustomHashMap.this.used;
  ...
  public void remove() {
   ...
   if ( pos < -1 ) {
    ...
    Double2ByteOpenCustomHashMap.this.remove( wrapped.getDouble( - pos - 2 ) );
    last = -1;
    return;
   }
   size--;
 ...
 private class FastEntryIterator extends MapIterator implements ObjectIterator<Double2ByteMap.Entry > {
  final BasicEntry entry = new BasicEntry ( ((double)0), ((byte)0) );
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2ByteMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
  ...
  public boolean remove( final Object o ) {
   ...
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) {
     Double2ByteOpenCustomHashMap.this.remove( e.getKey() );
     return true;
    }
    pos = ( pos + 1 ) & mask;
   }
