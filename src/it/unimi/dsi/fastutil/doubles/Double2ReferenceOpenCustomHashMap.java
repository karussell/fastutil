...
public class Double2ReferenceOpenCustomHashMap <V> extends AbstractDouble2ReferenceMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 private class MapIterator {
  ...
  {
   final boolean used[] = Double2ReferenceOpenCustomHashMap.this.used;
  ...
  public void remove() {
   ...
   if ( pos < -1 ) {
    ...
    Double2ReferenceOpenCustomHashMap.this.remove( wrapped.getDouble( - pos - 2 ) );
    last = -1;
    return;
   }
   size--;
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2ReferenceMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean remove( final Object o ) {
   ...
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) {
     Double2ReferenceOpenCustomHashMap.this.remove( e.getKey() );
     return true;
    }
    pos = ( pos + 1 ) & mask;
   }
