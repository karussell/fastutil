...
public class Double2CharOpenCustomHashMap extends AbstractDouble2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character get( final Double ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( strategy.equals( (key[ pos ]), ( k) ) ) ) return (Character.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private class MapIterator {
  ...
  {
   final boolean used[] = Double2CharOpenCustomHashMap.this.used;
  ...
  public void remove() {
   ...
   if ( pos < -1 ) {
    ...
    Double2CharOpenCustomHashMap.this.remove( wrapped.getDouble( - pos - 2 ) );
    last = -1;
    return;
   }
   size--;
 ...
 private class FastEntryIterator extends MapIterator implements ObjectIterator<Double2CharMap.Entry > {
  final BasicEntry entry = new BasicEntry ( ((double)0), ((char)0) );
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
  ...
  public boolean remove( final Object o ) {
   ...
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) {
     Double2CharOpenCustomHashMap.this.remove( e.getKey() );
     return true;
    }
    pos = ( pos + 1 ) & mask;
   }
