...
public class Float2LongOpenCustomHashMap extends AbstractFloat2LongMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Long get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( strategy.equals( (key[ pos ]), ( k) ) ) ) return (Long.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private class MapIterator {
  ...
  {
   final boolean used[] = Float2LongOpenCustomHashMap.this.used;
  ...
  public void remove() {
   ...
   if ( pos < -1 ) {
    ...
    Float2LongOpenCustomHashMap.this.remove( wrapped.getFloat( - pos - 2 ) );
    last = -1;
    return;
   }
   size--;
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2LongMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) return ( (value[ pos ]) == (((e.getValue()).longValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
  ...
  public boolean remove( final Object o ) {
   ...
   while( used[ pos ] ) {
    if ( ( strategy.equals( (key[ pos ]), (k) ) ) ) {
     Float2LongOpenCustomHashMap.this.remove( e.getKey() );
     return true;
    }
    pos = ( pos + 1 ) & mask;
   }
